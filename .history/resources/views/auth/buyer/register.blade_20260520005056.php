...
<body class="bg-[#F1E0B4] min-h-screen flex items-center justify-center px-4 py-8 relative overflow-x-hidden">
    ...
    <div class="bg-white/75 backdrop-blur-xl rounded-3xl shadow-xl flex flex-col md:flex-row w-full max-w-[700px] overflow-hidden relative z-10 my-4">
        ...
        <div class="flex-1 flex flex-col justify-center px-6 md:px-9 py-8">
            ...
            <form id="registerForm" action="{{ route('register.buyer.post') }}" method="POST" novalidate>
                @csrf
    ...
    <script>
        ...
        function updateStrengthBar(pw) {
            ...
            bars.forEach((bar, i) => {
                if (pw.length === 0) { bar.style.width = '0%'; bar.style.backgroundColor = ''; }
                else if (i < Math.ceil(score / 4 * 3)) { bar.style.width = '100%'; bar.style.backgroundColor = score <= 1 ? colors[0] : score <= 2 ? colors[1] : colors[2]; }
                else { bar.style.width = '0%'; }
            });
            if (pw.length === 0) { strengthLabel.textContent = ''; strengthLabel.className = 'block text-[10px] mt-0.5 pl-5 text-[#aaa]'; }
            else { const lvl = score <= 1 ? 1 : score <= 2 ? 2 : 3; strengthLabel.textContent = 'Kekuatan password: ' + labels[lvl]; strengthLabel.className = 'block text-[10px] mt-0.5 pl-5 ' + labelColors[lvl]; }
        }
