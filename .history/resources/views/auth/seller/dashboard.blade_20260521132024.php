...
<head>
    ...
    <style>
        * { font-family: 'Poppins', sans-serif; }
        ...
        .nav-link:hover { background: #FFF3EC; color: #FF5500; }
        .nav-link.active { background: #FF5500; color: #fff; }
        ...
        .stat-card-orange { background: linear-gradient(135deg, #FF5500 0%, #FF8C42 100%); }
        ...
        @media (min-width: 1024px) {
            ...
            .sidebar-collapsed .sidebar-divider-text { display: none !important; }
        ...
        ::-webkit-scrollbar-track { background: #f1e0b4; }
        ::-webkit-scrollbar-thumb { background: #cfc3a5; border-radius: 10px; }
...
{{-- ═══════════════ SIDEBAR ═══════════════ --}}
<aside id="sidebar"
    class="fixed top-0 left-0 h-full z-40 bg-white shadow-xl flex flex-col
           w-[220px]
    ...
    <div class="flex items-center justify-between px-5 pt-6 pb-4">
        ...
        <button id="sidebarToggleBtn" onclick="toggleSidebarDesktop()"
            ...
            <i id="sidebarToggleIcon" class="fa-solid fa-chevron-left text-[#FF5500] text-[11px] transition-transform duration-300"></i>
        </button>
        {{-- Mobile close --}}
    ...
    <div class="px-5 mb-4 sidebar-brand-text">
        <span class="bg-[#1a1a1a] text-white text-[11px] font-semibold px-3 py-1 rounded-md tracking-wide sidebar-badge">
            Seller
        </span>
    </div>
    ...
    <div class="mx-3 mb-5 rounded-xl bg-[#F1E0B4]/60 p-3 flex items-center gap-3">
        ...
        @else
            <div class="w-9 h-9 rounded-full bg-[#FF5500]/20 flex items-center justify-center ring-2 ring-[#FF5500]/30 shrink-0">
    ...
    <div class="p-4 border-t border-[#f0e9d8]">
        <form action="{{ route('logout') }}" method="POST">
            @csrf
            <button type="submit"
                class="w-full flex items-center gap-2.5 justify-center py-2.5 px-4 rounded-lg
...
<div class="flex-1 flex flex-col min-h-screen min-w-0">
    ...
    {{-- ── TOP NAVBAR ── --}}
    <header class="sticky top-0 z-20 bg-white/80 backdrop-blur-md shadow-sm px-5 lg:px-8 py-3 flex items-center justify-between">
        ...
        <div class="flex items-center gap-3">
            {{-- Notifikasi --}}
            <button class="relative w-9 h-9 rounded-full bg-[#f5f5f5] hover:bg-[#FFF3EC] flex items-center justify-center transition-colors"
        ...
        <div id="notifDropdown"
            ...
            <div class="px-4 py-3 border-b border-[#f0e9d8] flex items-center justify-between">
                ...
                <span class="text-[10px] text-[#FF5500] font-medium cursor-pointer hover:underline">Tandai semua dibaca</span>
            </div>
    ...
    <main class="flex-1 p-5 lg:p-8 space-y-6">
        ...
        {{-- Flash success --}}
        @if(session('success'))
            <div class="bg-green-50 border border-green-200 text-green-700 px-4 py-3 rounded-xl text-[13px] flex items-center gap-2.5">
            ...
            <div class="bg-white rounded-2xl p-5 shadow-sm flex flex-col">
                ...
                <div class="flex-1 space-y-3 overflow-y-auto max-h-[220px] pr-1">
                    @forelse($pesananTerbaru ?? [] as $pesanan)
                        <div class="flex items-center gap-3 p-2.5 rounded-xl hover:bg-[#FFF3EC] transition-colors cursor-pointer">
                            ...
                            <div class="flex-1 min-w-0">
                                <p class="text-[12px] font-medium text-[#1a1a1a] truncate">{{ $pesanan->nama_produk ?? 'Produk' }}</p>
                ...
                <table class="w-full">
                    <thead>
                        <tr class="border-b border-[#f0e9d8]">
                            ...
                            <th class="text-left text-[11px] text-[#aaa] font-medium pb-2.5 hidden sm:table-cell">Kategori</th>
                            ...
                            <tr class="hover:bg-[#FFF3EC]/50 transition-colors">
                                ...
                                <td class="py-3">
                                    <div class="flex items-center gap-2.5">
                                        ...
                                        @endif
                                        <div>
                                            <p class="text-[12px] font-medium text-[#1a1a1a] leading-tight">{{ $produk->nama }}</p>
                                            <p class="text-[10px] text-[#aaa]">Rp {{ number_format($produk->harga, 0, ',', '.') }}</p>
                                        </div>
                                    </div>
                                </td>
                                <td class="py-3 hidden sm:table-cell">
                                    <span class="text-[11px] bg-orange-50 text-[#FF5500] px-2 py-0.5 rounded-full font-medium">
                                        {{ $produk->kategori ?? '-' }}
                                    </span>
                                </td>
                                <td class="py-3 text-right">
                                    <span class="text-[13px] font-semibold text-[#1a1a1a]">{{ $produk->total_terjual ?? 0 }}</span>
                                </td>
                                <td class="py-3 text-right pr-1">
                                    <span class="text-[12px] font-semibold text-green-600">
                                        Rp {{ number_format(($produk->harga ?? 0) * ($produk->total_terjual ?? 0), 0, ',', '.') }}
                                    </span>
                                </td>
                            </tr>
                        @empty
...
const chartDataTahunan = @json($chartTahunan ?? [
    'labels' => ['2020','2021','2022','2023','2024','2025','2026','2027'],
    'data'   => [32, 29, 17, 14, 11, 9, 6, 2],
]);
const chartDataBulanan = @json($chartBulanan ?? [
