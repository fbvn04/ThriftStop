...
<head>
    ...
    <title>Daftar Seller - ThriftStop</title>
...
<body class="bg-[#F1E0B4] min-h-screen flex items-center justify-center px-4 py-8 relative overflow-x-hidden">
    ...
    <div class="absolute -left-28 top-0 h-full w-[40%] overflow-hidden flex items-start justify-start pointer-events-none">
        <img src="{{ asset('images/thrift-login.png') }}" alt="ThriftStop Fashion"
    ...
    <div class="bg-white/75 backdrop-blur-xl rounded-3xl shadow-xl flex flex-col md:flex-row w-full max-w-[700px] overflow-hidden relative z-10 my-4">
        ...
        <div class="hidden md:flex w-[38%] flex-col justify-center px-8 py-10 shrink-0">
            ...
            <span class="mt-2 mb-3 inline-block bg-[#1a1a1a] text-white text-[12px] font-semibold px-3 py-1 rounded-md tracking-wide w-fit">
                Seller
            </span>
            <p class="text-[12px] text-[#555] leading-relaxed">
                Buka toko thrift-mu dan jangkau ribuan pembeli di seluruh Indonesia.
            </p>
        </div>
        ...
        <div class="flex-1 flex flex-col justify-center px-6 md:px-9 py-8">
            ...
            <div class="md:hidden mb-5">
                ...
                <span class="mt-1 inline-block bg-[#1a1a1a] text-white text-[11px] font-semibold px-2.5 py-0.5 rounded-md tracking-wide">Seller</span>
            </div>

            <h2 class="text-[18px] font-semibold text-[#1a1a1a] mb-1">Daftar Akun Seller</h2>
            <p class="text-[11px] text-[#888] mb-5">Lengkapi data untuk membuka toko kamu</p>
            ...
            <form id="registerForm" action="{{ route('register.seller.post') }}" method="POST" novalidate>
                @csrf
                ...
                {{-- Nama Toko --}}
                <div class="mb-1">
                    <div id="namaTokenBorder" class="flex items-center border-b border-[#000] transition-colors duration-200">
                        <i class="fa-solid fa-store text-[#000] text-[13px] mr-2 shrink-0"></i>
                        <input type="text" id="nama_toko" name="nama_toko" placeholder="Nama toko kamu"
                            value="{{ old('nama_toko') }}"
                            class="flex-1 bg-transparent py-2.5 text-[13px] text-[#000] placeholder-[#aaa] border-none ring-0 focus:ring-0">
                    </div>
                    <span id="namaTokoError" class="block text-[10px] text-red-500 min-h-[14px] mt-0.5 pl-5"></span>
                </div>
                ...
                <div class="mb-1">
                    <div id="emailBorder" class="flex items-center border-b border-[#000] transition-colors duration-200">
                        ...
                        <input type="email" id="email" name="email" placeholder="Email aktif kamu"
                ...
                <div class="mb-1">
                    <div id="hpBorder" class="flex items-center border-b border-[#000] transition-colors duration-200">
                        ...
                        <input type="tel" id="hp" name="hp" placeholder="8xx xxxx xxxx"
                            value="{{ old('hp') }}" autocomplete="tel" maxlength="13"
                            class="flex-1 bg-transparent py-2.5 text-[13px] text-[#000] placeholder-[#aaa] border-none ring-0 focus:ring-0">
                    </div>
                ...
                <div class="mb-1">
                    ...
                    <div class="flex gap-1 mt-1.5 pl-5">
                        ...
                        <div class="h-1 flex-1 rounded-full bg-gray-200 overflow-hidden"><div id="bar3" class="h-full w-0 strength-bar rounded-full"></div></div>
                    </div>
                ...
                <div class="mb-1">
                    <div id="konfirmBorder" class="flex items-center border-b border-[#000] transition-colors duration-200">
                        <i class="fa-solid fa-lock text-[#000] text-[13px] mr-2 shrink-0"></i>
                        <input type="password" id="konfirm" name="password_confirmation" placeholder="Ulangi password"
                            autocomplete="new-password"
                            class="flex-1 bg-transparent py-2.5 text-[13px] text-[#000] placeholder-[#aaa] border-none ring-0 focus:ring-0">
                ...
                <button type="submit" id="submitBtn" disabled
                    ...
                    <i class="fa-solid fa-store mr-1.5 text-[13px]"></i> Buka Toko
                </button>
            </form>

            <p class="text-center text-[11px] text-[#888] mt-4">
                Sudah punya akun?
                <a href="{{ route('login.seller') }}" class="text-orange-500 font-semibold hover:underline">Masuk sebagai Seller</a>
            </p>

            {{-- Switch to buyer --}}
            <div class="flex items-center gap-3 mt-4">
                <div class="flex-1 h-px bg-[#cfc3a5]"></div>
                <span class="text-[10px] text-[#999]">atau daftar sebagai</span>
                <div class="flex-1 h-px bg-[#cfc3a5]"></div>
            </div>
            <a href="{{ route('register.buyer') }}"
                class="flex items-center justify-center gap-2 w-full mt-3 border border-[#cfc3a5] hover:border-orange-400 hover:text-orange-500 text-[#555] text-[13px] font-medium py-2.5 rounded-lg transition-all duration-200">
    ...
    <script>
        const fields = {
            nama:     { input: document.getElementById('nama'),      border: document.getElementById('namaBorder'),      error: document.getElementById('namaError') },
            namaToko: { input: document.getElementById('nama_toko'), border: document.getElementById('namaTokenBorder'), error: document.getElementById('namaTokoError') },
            username: { input: document.getElementById('username'),  border: document.getElementById('usernameBorder'),  error: document.getElementById('usernameError') },
            email:    { input: document.getElementById('email'),     border: document.getElementById('emailBorder'),     error: document.getElementById('emailError') },
            hp:       { input: document.getElementById('hp'),        border: document.getElementById('hpBorder'),        error: document.getElementById('hpError') },
            password: { input: document.getElementById('password'),  border: document.getElementById('passwordBorder'),  error: document.getElementById('passwordError') },
            konfirm:  { input: document.getElementById('konfirm'),   border: document.getElementById('konfirmBorder'),   error: document.getElementById('konfirmError') },
        };
        ...
        function setError(key, msg) {
            fields[key].border.classList.remove('border-green-400');
            fields[key].border.classList.add('border-red-400');
            fields[key].error.textContent = msg;
        }
        function setValid(key) {
            fields[key].border.classList.remove('border-red-400');
            fields[key].border.classList.add('border-green-400');
            fields[key].error.textContent = '';
        }

        function validateNama() {
            const v = fields.nama.input.value.trim();
            if (!v)          { setError('nama', 'Nama tidak boleh kosong.'); return false; }
        ...
        function validateNamaToko() {
            const v = fields.namaToko.input.value.trim();
            if (!v)           { setError('namaToko', 'Nama toko tidak boleh kosong.'); return false; }
            if (v.length < 3) { setError('namaToko', 'Nama toko minimal 3 karakter.'); return false; }
            if (v.length > 50){ setError('namaToko', 'Nama toko maksimal 50 karakter.'); return false; }
            setValid('namaToko'); return true;
        }
        ...
        function validateEmail() {
            ...
            if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(v)) { setError('email', 'Format email tidak valid.'); return false; }
        ...
        function updateStrengthBar(pw) {
            ...
            bars.forEach((bar, i) => {
                if (!pw) { bar.style.width = '0%'; bar.style.backgroundColor = ''; return; }
                bar.style.width = i < Math.ceil(score / 4 * 3) ? '100%' : '0%';
                bar.style.backgroundColor = score <= 1 ? colors[0] : score <= 2 ? colors[1] : colors[2];
            });
            if (!pw) { strengthLabel.textContent = ''; strengthLabel.className = 'block text-[10px] mt-0.5 pl-5 text-[#aaa]'; return; }
        ...
        function validatePassword() {
            ...
            if (getStrength(v) < 2) { setError('password', 'Terlalu lemah. Tambahkan huruf besar atau angka.'); return false; }
            setValid('password');
        ...
        function validateKonfirm() {
            ...
            if (!v)                          { setError('konfirm', 'Konfirmasi password wajib diisi.'); return false; }
            if (v !== fields.password.input.value) { setError('konfirm', 'Password tidak cocok.'); return false; }
            setValid('konfirm'); return true;
        }

        function checkAll() {
            const ok = validateNama() && validateNamaToko() && validateUsername()
                    && validateEmail() && validateHp() && validatePassword() && validateKonfirm();
            submitBtn.disabled = !ok;
        }

        Object.keys(fields).forEach(key => {
            fields[key].input.addEventListener('input', checkAll);
            fields[key].input.addEventListener('blur', checkAll);
        });
        ...
        ['nama','namaToko','username','email','hp'].forEach(key => {
