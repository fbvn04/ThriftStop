...
<body class="bg-[#F0E6C8] min-h-screen flex flex-col">

    {{-- Top dark bar --}}
    <div class="fixed top-0 left-0 right-0 h-4 bg-[#1a1a1a] z-50"></div>
    {{-- Bottom dark bar --}}
    <div class="fixed bottom-0 left-0 right-0 h-4 bg-[#1a1a1a] z-50"></div>
    ...
    <div class="flex-1 flex items-center justify-center px-4 py-16">
        ...
        <div class="bg-white/75 backdrop-blur-xl rounded-3xl shadow-xl flex w-full max-w-[680px] overflow-hidden">
            ...
            <div class="relative w-[42%] min-h-[340px] overflow-hidden rounded-2xl">
                <img
                ...
                >
                {{-- Overlay --}}
                <div class="absolute inset-0 bg-[#F0E6C8]/45"></div>

                {{-- Brand text --}}
                <div class="absolute bottom-8 left-6 z-10">
                    <h1 class="text-[28px] font-extrabold leading-none">
                        <span class="text-orange-500">Thrift</span><span class="text-[#1a1a1a]">Stop</span>
                    </h1>
                    <p class="text-[11px] text-[#3a3a3a] mt-1.5 leading-snug font-normal">
    ...
    <script>
        ...
        function validateUsername() {
            ...
            if (val === '') {
                setError(usernameBorder, usernameError, 'Username tidak boleh kosong.');
                return false;
            } else if (val.length < 3) {
                setError(usernameBorder, usernameError, 'Username minimal 3 karakter.');
                return false;
            } else if (/\s/.test(val)) {
                setError(usernameBorder, usernameError, 'Username tidak boleh mengandung spasi.');
                return false;
            }
            setValid(usernameBorder, usernameError);
            return true;
        }
        ...
        function validatePassword() {
            ...
            if (val === '') {
                setError(passwordBorder, passwordError, 'Password tidak boleh kosong.');
                return false;
            } else if (val.length < 6) {
                setError(passwordBorder, passwordError, 'Password minimal 6 karakter.');
                return false;
            }
            setValid(passwordBorder, passwordError);
            return true;
        }
        ...
        function checkForm() {
            ...
            submitBtn.disabled = !(usernameOk && passwordOk);
        }
        ...
        // ── Aktifkan tombol jika ada old input dari server ────
