...
<head>
    ...
    <title>ThriftStop - Kelola Produk</title>
    ...
    <style>
        * {
        ...
        .incomplete-checklist-item .check-icon {
            width: 20px;
            height: 20px;
            border-radius: 50%;
            display: flex;
...
<body>
    @if(!$tokoLengkap)
        <div class="incomplete-overlay">
            <div class="incomplete-card">
                <div class="incomplete-icon-wrap">
                    <i class="fa-solid fa-store-slash" style="font-size:28px;color:#FF5500;"></i>
                </div>
                <h2>Lengkapi Profil Toko Dulu</h2>
                <p>Sebelum bisa mengelola produk, lengkapi profil tokomu terlebih dahulu ya!</p>
                ...
                <div class="incomplete-checklist">
                    ...
                    <div class="incomplete-checklist-item">
                        <span
                            class="check-icon {{ !empty($toko->provinsi_id) && !empty($toko->kota_id) && !empty($toko->kecamatan_id) ? 'done' : 'undone' }}">
                            <i
                                class="fa-solid {{ !empty($toko->provinsi_id) && !empty($toko->kota_id) && !empty($toko->kecamatan_id) ? 'fa-check' : 'fa-xmark' }}"></i>
                        </span>
    ...
    <div id="mainWrap" class="{{ !$tokoLengkap ? 'profile-incomplete-blur' : '' }}">
        <header
                ...
                <div class="product-grid" id="productGrid">
                    @foreach($produks as $produk)
                        <div class="product-card" data-id="{{ $produk->id }}" data-nama="{{ $produk->nama_produk }}"
                            ...
                            onclick="window.location='{{ route('seller.produk.detail', $produk->id) }}'">
    ...
    <div id="logoutModal" class="fixed inset-0 z-50 flex items-center justify-center hidden">
        ...
        <div class="relative bg-white rounded-2xl shadow-2xl p-6 w-[300px] mx-4 text-center"
            ...
            <p class="text-[12px] text-[#888] mt-1.5 mb-5 leading-relaxed">Kamu akan keluar dari akun seller
                ThriftStop. Yakin ingin melanjutkan?</p>
